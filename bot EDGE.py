...
#Lego Yoda Death Sound Lottery
async def on_voice_server_update : #Updates Code Every Time A User Leaves Or Joins
    if user.id(arg) == 1 : #Defines How Many People Are In A VC At Once. Updates Every Time A User Leaves Or Joins
        while (Lottery != 20):
            Lottery = (random.randint(19,20)) #Random Number Between _-_, Defines Chances of bot joining VC
            print str(Lottery)
            sleep(10) #Waits X Amount Of Seconds Before Picking Another Number
        async def join(ctx):
            voice_channel = user.channel #Joins Channel That User Is In
            vc = await VoiceChannel.connect(voice_channel)
            sleep(1)
            vc.play(discord.FFmpegPCMAudio('LYDS.mp3'), after=lamda: print('done')) #Plays .mp3 File
            if is_playing():
                return
            elif stop():
                vc = await disconnect
